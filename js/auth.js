        ...
        try {
            ...
            if (savedUser) {
                try {
                    ...
                    // Disparar evento para manejar el cambio de autenticación
                    document.dispatchEvent(new CustomEvent('authChange', { detail: { user: this.currentSession, session: this.currentSession } }));
                } catch (e) {
                    console.error('Error al recuperar sesión:', e);
                    localStorage.removeItem('biblio_user');
                    // Disparar evento para manejar el cambio de autenticación
                    document.dispatchEvent(new CustomEvent('authChange', { detail: { user: null, session: null } }));
                }
            } else {
                // No hay sesión, disparar evento para mostrar modal de autenticación
                document.dispatchEvent(new CustomEvent('authChange', { detail: { user: null, session: null } }));
            }
